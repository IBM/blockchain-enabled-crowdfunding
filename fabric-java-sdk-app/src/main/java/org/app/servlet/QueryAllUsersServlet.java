...
public class QueryAllUsersServlet extends HttpServlet {
	...
	*/
	...
	/*
	 * This method invokes queryAllUsers method of smart contract
	 */
	private static String queryNeed(JSONObject req) throws Exception{
		...
		QueryByChaincodeRequest queryRequest3 = hfClient.newQueryProposalRequest();
		queryRequest3.setChaincodeID(ccid);
		queryRequest3.setFcn("queryAllUsers");

		Collection<ProposalResponse> responses3Query = channel.queryByChaincode(queryRequest3);
		for (ProposalResponse pres : responses3Query) {
			stringResponse3 = new String(pres.getChaincodeActionResponsePayload());
			Logger.getLogger(QueryAllUsersServlet.class.getName()).log(Level.INFO, "Query response: " + stringResponse3);

		}
		
		
		return stringResponse3;
	}
